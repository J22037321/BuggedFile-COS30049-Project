...
path = "Dataset Results/AU-PEMAL2025v2_Cleaned/"
os.makedirs(path, exist_ok=True)


#Loading data into a DataFrame
malwareset=pd.read_csv('Raw Datasets/AU-PEMal-2025-V2.csv')

#Remove identification hashvalues, duplicate samples, same value rows, and stats from dynamic analysis, also removing category and family of malware to make things simpler
malwareset.drop_duplicates(subset=['sha1','Checksum'],keep='first', inplace=True)
malwareset.drop(['sha1','processes_malicious','processes_monitored','processes_suspicious','total_procsses','files_malicious','files_suspicious','files_unknown','files_text',
                 'registry_total','registry_read','registry_write','registry_delete','network_dns','network_http','network_connections','Category','Family','Checksum'], axis=1, inplace=True)

#Class benign samples as 0 and non-benign as 1
malwareset['Class'] = malwareset['Class'].astype('category')
malwareset['Class'] = malwareset['Class'].map({'Benign':0, 'Malware':1})
...
counts = malwareset['Class'].value_counts()
print("Benign samples:", counts[0])
print("Malware samples:", counts[1])

with open("Dataset Results/AU-PEMAL2025v2_Cleaned/AU-PEMAL2025v2_Cleaned_output.txt","w") as file:
...
plt.xlabel("Class")
plt.ylabel("Count")
plt.savefig("Dataset Results/AU-PEMAL2025v2_Cleaned/Class Distribution.pdf" , bbox_inches="tight")
plt.tight_layout()
plt.show()
...
#Features and labels
X = malwareset.drop('Class', axis=1)
y = malwareset['Class']


data_input = malwareset.drop('Class', axis=1).values
labels= malwareset['Class'].values
...
with open("Dataset Results/AU-PEMAL2025v2_Cleaned/AU-PEMAL2025v2_Cleaned_output.txt","a") as file:
    print(f"The data has {data_input.shape[0]} samples with {data_input.shape[1]} usable features.\n"
      f"After evaluating with an Extra Trees Forest model the most important features out of the {data_input.shape[1]} before are the {data_input_new.shape[1]} features below:\n",file=file)
...
sns.barplot(x=importance, y=malwareset.drop('Class', axis=1).columns)
plt.title("Feature Importances (Extra Trees)")
plt.ylabel("Features")
plt.savefig("Dataset Results/AU-PEMAL2025v2_Cleaned/Feature Importances (Extra Trees).pdf" , bbox_inches="tight")
plt.tight_layout()
plt.show()
...
plt.ylabel("Actual")
plt.savefig("Dataset Results/AU-PEMAL2025v2_Cleaned/Confusion_Matrix.pdf" , bbox_inches="tight")
plt.tight_layout()
plt.show()
...
#Regression model
X = malwareset[['apis']]   # features
y = malwareset['dlls_calls']                # continuous target
...
plt.xlabel("apis")
plt.ylabel("dlls_calls")
plt.title("Linear Regression on AU-PEMal dataset")
plt.savefig("Dataset Results/AU-PEMAL2025v2_Cleaned/Linear_Regression.pdf" , bbox_inches="tight")
plt.tight_layout()
plt.legend()
plt.show()
...
#path = "New Outputs/"
#os.makedirs(path, exist_ok=True)
#malwareset.to_csv('New Outputs/AU-PEMal2025v2_Cleaned.csv', index=False)